...
- 解答のたびに、既存の習熟度スコア(0〜100)を Part1 §2.6 のルールでAnki評価(Again/Hard/Good/Easy)に変換する
...
- `GET /api/words`(§2.4)の出題選定では、`due_at`が現在時刻以前(またはNULL=未レビュー)の単語を「復習期限が来た単語」として優先的に選出する
- 実装は`backend/.../service/FsrsScheduler.java`(スコア→評価変換、Card⇔DBフィールドの相互変換、出題可否判定)
...
- 出題選定は1-3節のFSRS復習期限を優先する:該当レベルの単語のうち`due_at`が現在時刻以前(未レビュー含む)のものを優先的に、不足分は復習期限前の単語からランダムに補って**合計10件**を返す(1クイズセッション分)。該当レベルの単語が10件未満の場合は存在する分すべてを返す。
